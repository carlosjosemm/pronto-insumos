...
* **Current Functions & Libraries:**
  ...
  * [`track-order.ts`](file:///c:/Users/ecmv2/Documents/PRONTO/api/track-order.ts): Public order tracking endpoint securing order consultations via canonical Order ID (`PRONTO-XXXXXX`) and Chilean Modulo 11 customer RUT verification. Bypasses client-side Firestore read restrictions safely on the server and returns a sanitized fulfillment progress timeline without exposing internal payment secrets or database internals.
  * [`upload-voucher.ts`](file:///c:/Users/ecmv2/Documents/PRONTO/api/upload-voucher.ts): Bank transfer voucher intake endpoint validating payload size (<= 5MB), MIME types (PDF, PNG, JPEG), and customer RUT before persisting the voucher and advancing the order state to `'TRANSFERENCIA_COMPROBANTE_SUBIDO'` in Firestore Admin.
