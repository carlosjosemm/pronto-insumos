...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!dataUrl) {
      return res.status(400).json({ error: 'No se ha adjuntado el archivo de comprobante (dataUrl ausente).' })
    }
    ...
    if (!adminDb) {
      console.warn('Firestore Admin not available. Returning simulated voucher upload response.')
