...
**Progressive catalog reveal (Task 8.7).** `ProductList` no longer mounts the whole catalog at once. `src/hooks/useIncrementalReveal.ts` owns the reveal state: page size is **16** (`PRODUCTS_PAGE_SIZE`), `revealMore()` advances one page, and the count is **derived** (`Math.min(visibleCount, max)`) so a shrinking result set clamps without an effect — `react-hooks/set-state-in-effect` forbids reset effects here. Reset-on-filter-change is achieved by **remount**: `App` renders `<ProductList key={catalogRequestKey} …>`, matching the codebase's "scoped by remount, not by reset effects" convention (§2.1). Below the grid, a `.load-more-row` carries (a) an `aria-hidden` `.load-more-sentinel` observed by an `IntersectionObserver` (`rootMargin: 200px`, guarded for runtimes without the API — jsdom has none) that auto-reveals, (b) the primary keyboard-reachable `Cargar más insumos` button — the observer is a convenience, never a replacement — and (c) a `role="status" aria-live="polite"` `Mostrando N de M` count. Both the button and the count disappear once the end of the list is reached. Cards stay keyed by `product.id`, so already-revealed cards keep their DOM nodes and the `product-card-entrance` animation runs **once per card** across reveals (asserted by DOM-identity in `ProductList.test.tsx`); a filter change intentionally replays it via the remount. The grid collapses to a **single column below 560px** (`@media (max-width: 560px)` in `src/index.css`) — the 2-column phone layout wrapped card details unreadably; 561–768px keeps 2 columns. That media query is guarded by `src/tests/styles/storefrontCss.test.ts` because jsdom cannot evaluate media queries.
